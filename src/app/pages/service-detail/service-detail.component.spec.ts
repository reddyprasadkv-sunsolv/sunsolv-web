...
describe('ServiceDetailComponent', () => {
  ...
  it('keeps the other four individual service routes on their existing definitions', () => {
    const remainingServiceSlugs = Object.keys(serviceRouteData).filter(
      (slug) =>
        slug !== 'it-consulting' && slug !== 'digital-transformation' && slug !== 'cloud-solutions',
    );
    const routedServicePaths = routes
      .map((route) => route.path)
      .filter((path): path is string => path?.startsWith('services/') === true);

    expect(remainingServiceSlugs).toHaveLength(4);
